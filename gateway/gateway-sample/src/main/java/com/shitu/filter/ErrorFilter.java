...
import com.shitu.handler.BodyHackerHttpResponseDecorator;
import com.shitu.handler.BodyHandlerFunction;
...
public class ErrorFilter implements GatewayFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 定义写入body的逻辑
        BodyHandlerFunction deleagte = (resp, body) -> Flux.from(body)
                .flatMap(orgBody -> {
                    // 原始的response body
        ...
        BodyHackerHttpResponseDecorator responseDecorator =
                new BodyHackerHttpResponseDecorator(deleagte, exchange.getResponse());
